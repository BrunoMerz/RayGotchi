...
void IconRenderer::renderAndDisplay(const __FlashStringHelper* iconFileName, int delayAfter, byte cleanUp, int16_t x_pos, int16_t y_pos) {
  char *fb;
  ...
  if(!fb) {
    ...
    myfs->writeLog(F("IconRenderer: malloc failed\n"));
    return;
  }
  int lng = myfs->readFile(tmp, fb);
  if(!lng){
    DEBUG_PRINTLN("renderAndDisplay: file not found -> "+iconFileName);
    myfs->writeLog(F("IconRenderer: file not found\n"));
    return;
  }
